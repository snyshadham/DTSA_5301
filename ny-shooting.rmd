---
title: "NYC Shooting Incident Analysis Report"
...
```{r summarize raw shooting data, echo=TRUE}
summary(nydata)
...
```{r examine and prepare data, echo=TRUE}
...
```{r tabulate demographics victim NYC, echo=TRUE}
...
```{r tabulate demographics perpetrator NYC, echo=TRUE}
...
```{r visualize data NYC, echo=TRUE}
NY_Totals <- ny %>%
...
```{r visualize data borough, echo=TRUE}
Boro_Totals <- ny %>%
...
```{r get map of nyc based on borders in data, echo=TRUE, message=FALSE, results=TRUE}
...
```{r map of nyc across years, echo=TRUE}
ggmap(nyc_map) +
...
```{r get raw unemployment data, echo=TRUE}
...
```{r recode unemployment data, echo=TRUE}
unemp <- empdata %>% 
